<template>
  <div class="card-container">
    <a-card title="角色权限" :bordered="false" style="min-height: 100%">
      <div style="display: flex;justify-content: space-between;align-items:stretch">
        ...
        <a-card style="width: 31%" :bodyStyle="{padding:'18px'}">
          <div style="height: 40px;display: flex;align-items: center;background-color: #f2f2f2;padding-left: 20px;font-size: 15px;color: rgba(0,0,0,.85)">当前角色拥有的权限如下：</div>
  ...
  export default {
    ...
    methods:{
      getRoleList(){
        get(`${BASE_URL}/uip/smRole/queryRoleList?pageNumber=-1&pageSize=-1`).then(res=>{
          if(res.resCode===1){
            ...
            this.loading1=false;
            this.tree1Data=res.data.records;
          }
        })
      },
      ...
      setPermission(){
         ...
         post(`${BASE_URL}/uip/smAuthority/authorizedRole`,params).then(res=>{
