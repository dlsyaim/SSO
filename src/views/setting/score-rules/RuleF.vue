    ...
    <a-card title="水质评分细则" :bordered="false" style="min-height: 100%">
      ...
      <div style="display: flex;justify-content: space-between">
        <div style="display: flex;align-items: center;">
          ...
          <span>水质类别：</span>
          <a-select class="default-select-width" style="margin-right: 40px" v-model="waterQualityType"
                    placeholder="请选择水质类别">
            <a-select-option v-for="(value,key) in waterQualityTypeEnum" :value="key" :key="key">{{value}}</a-select-option>
          </a-select>
    ...
    >
      <a-form :form="form">
        <a-row>
          ...
          <a-col span="12">
            <a-form-item v-bind="formLayout" label="水质类别">
              <a-select v-decorator="['waterQualityType']" placeholder="请选择水质类别">
                <a-select-option v-for="(value,key) in waterQualityTypeEnum" :value="key" :key="key">{{value}}</a-select-option>
              </a-select>
            </a-form-item>
          </a-col>
          <a-col span="12">
            <a-form-item v-bind="formLayout" label="得分系数">
              <a-input v-decorator="['scoreCoefficient']" placeholder="请输入得分系数"></a-input>
            </a-form-item>
          </a-col>
          <a-col span="12">
            <a-form-item v-bind="formLayout" label="修正系数">
              <a-input v-decorator="['factorCoefficient']" placeholder="请输入修正系数"></a-input>
            </a-form-item>
          </a-col>
          <a-col span="12">
...
<script>
  ...
  const columns = [
    ...
    {title: '水质类别', dataIndex: 'waterQualityTypeName'},
    {title: '得分系数', dataIndex: 'scoreCoefficient'},
    {title: '修正系数', dataIndex: 'factorCoefficient'},
    ...
    methods: {
      getList() {
        ...
        const params = {
          ...
          waterQualityType: this.waterQualityType,
          ruleId: this.id
        };
        get(`${BASE_URL}/statistic/v1/waterAssess/listWaterAssess`, params).then(res => {
          this.loading = false;
          if (res.resCode === 1) {
      ...
      handleData(list) {
        list.forEach((item, index) => {
          ...
          item.waterQualityTypeName=this.waterQualityTypeEnum[item.waterQualityType];
        });
        this.list = list;
      },
      resetSearchCondition() {
        this.paramType = undefined;
        this.waterQualityType = undefined;
      },
      handleTableChange(e) {
        this.pagination = e;
        this.getList();
          ...
          if(method==='modify'){
            ...
            this.$nextTick(()=>{
              this.form.setFieldsValue({
                ...
                waterQualityType: this.selected.waterQualityType,
                scoreCoefficient:this.selected.scoreCoefficient,
                factorCoefficient: this.selected.factorCoefficient,
      ...
      add(value){
        ...
        post(`${BASE_URL}/statistic/v1/waterAssess/addWaterAssess`,params).then(res=>{
      ...
      edit(value){
        ...
        put(`${BASE_URL}/statistic/v1/waterAssess/updateWaterAssess`,params).then(res=>{
          this.isSaveLoading=false;
          if(res.resCode===1){
