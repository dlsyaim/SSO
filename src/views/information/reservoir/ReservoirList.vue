<template>
  <div class="card-container">
    <a-card title="水库管理" :bordered="false" style="min-height: 100%">
      <div style="display: flex;align-items: center;justify-content: space-between">
        <div style="display: flex;align-items: center">
          <span>水库名称：</span>
          <a-input v-model="reservoirName" placeholder="请输入水库名称" class="default-select-width"
                   style="margin-right: 40px"></a-input>
          <span>水库类型：</span>
          <a-select v-model="reservoirType" placeholder="请水库河流类型" class="default-select-width" style="margin-right: 40px">
            <a-select-option v-for="item in reservoirTypeList" :key="item.id" :value="item.id">{{item.typeName}}</a-select-option>
          </a-select>
          <span>所属区域：</span>
        ...
        <a-button type="primary" @click="$router.push('/information/reservoir/add')">
...
<script>
  ...
  const columns = [
    ...
    {title: '水库名称', dataIndex: 'reservoirName'},
    {title: '水库类型', dataIndex: 'reservoirTypeName'},
    {title: '所属区域', dataIndex: 'regionName'},
    {title: '水库岸别', dataIndex: 'reservoirShoreName'},
    {title: '水库面积', dataIndex: 'acreage'},
  ...
  export default {
    ...
    methods: {
      getList() {
        this.loading = true;
        const params = {
          reservoirName: this.reservoirName,
          reservoirType:this.reservoirType,
          regionCode: this.region.id,
          page: this.pagination.current,
          size: this.pagination.pageSize
        };
        get(`${BASE_URL}/watersource/v1/reservoir/list`, params).then(res => {
          this.loading = false;
          if (res.resCode === 1) {
      ...
      handleTableClick(e) {
        ...
        if (id && method) {
          ...
          if (method === 'detail') {
            this.$router.push({path: '/information/reservoir/detail', query: {id: this.selected.id}});
          } else if (method === 'modify') {
            this.$router.push({path: '/information/reservoir/edit', query: {id: this.selected.id}});
      ...
      deleteItem() {
        this.loading=true;
        deleteRequest(`${BASE_URL}/watersource/v1/reservoir/delete?id=${this.selected.id}`).then(res=>{
          this.loading=false;
          if(res.resCode===1){
