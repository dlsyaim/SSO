<template>
  <div class="card-container">
    <a-card title="新增水库" :bordered="false" style="min-height: 100%">
      ...
      <div style="display: flex;justify-content: space-between;">
        <div style="width: 70%;" :style="{display:isMapMax?'none':'block'}">
          <a-form :form="form">
            <a-row>
              <a-col span="8">
                <a-form-item v-bind="formLayout" label="水库名称">
                  <a-input placeholder="请输入水库名称"
                           v-decorator="['reservoirName',{rules:[{required:true,message:'请输入水库名称'},reservoirNameAsyncValidator],validateFirst:true,validateTrigger:'blur'}]"></a-input>
                </a-form-item>
              </a-col>
              <a-col span="8">
              ...
              <a-col span="8">
                <a-form-item v-bind="formLayout" label="水库类型">
                  <a-select v-decorator="['reservoirType']" placeholder="请选择水库类型">
                    <a-select-option v-for="item in reservoirTypeList" :key="item.id" :value="item.typeValue">{{item.typeName}}
                    </a-select-option>
                  </a-select>
                </a-form-item>
              </a-col>
              <a-col span="8">
                <a-form-item v-bind="formLayout" label="水库岸别">
                  <a-select v-decorator="['reservoirBank']" placeholder="请选择水库岸别">
                    <a-select-option v-for="item in reservoirShoreList" :key="item.id" :value="item.typeValue">
                      {{item.typeName}}
                    </a-select-option>
                  </a-select>
                </a-form-item>
              ...
              <a-col span="8">
                <a-form-item v-bind="formLayout" label="管理单位">
                  <a-input placeholder="请输入管理单位" v-decorator="['managementUnit']"></a-input>
                </a-form-item>
              </a-col>
              <a-col span="8">
                <a-form-item v-bind="formLayout" label="水库面积">
                  <a-input placeholder="水库面积可由绘制得出" addonAfter="km²" v-decorator="['acreage']"></a-input>
                </a-form-item>
              </a-col>
              <a-col span="8">
                <a-form-item v-bind="formLayout" label="水库经度">
                  <a-input placeholder="水库经度可由绘制得出" v-decorator="['centerLongitude']"></a-input>
                </a-form-item>
              </a-col>
              <a-col span="8">
                <a-form-item v-bind="formLayout" label="水库纬度">
                  <a-input placeholder="水库纬度可由绘制得出" v-decorator="['centerLatitude']"></a-input>
                </a-form-item>
              </a-col>
              <a-col span="8">
...
<script>
  ...
  const reservoirNameAsyncValidator = {
    validator: (rules, value, callback) => {
      get(`${BASE_URL}/watersource/v1/reservoir/isRepeat?name=${value}`).then(res => {
        if (res.resCode !== 1) {
          callback(new Error());
        }else {
          callback();
  ...
  export default {
    ...
    mounted() {
      ...
      this.getReservoirTypeList();
      this.getReservoirShoreList();
      this.initMap();
    },
    methods: {
      initMap() {
      ...
      getReservoirTypeList() {
        get(`${BASE_URL}/watersource/v1/river/riverType`, {type: '108'}).then(res => {
          if (res.resCode === 1) {
            this.reservoirTypeList = res.data;
          }
        })
      },
      getReservoirShoreList() {
        get(`${BASE_URL}/watersource/v1/river/riverType`, {type: '114'}).then(res => {
          if (res.resCode === 1) {
            this.reservoirShoreList = res.data;
          }
        })
      },
      addForm() {
      ...
      save() {
        ...
        this.form.validateFields((err, value) => {
          if (!err&&!managerFormHasError) {
            ...
            post(`${BASE_URL}/watersource/v1/reservoir/add`, null, data).then(res => {
              this.isSaveLoading = false;
              if (res.resCode === 1) {
                this.$message.success('新增成功');
                setTimeout(() => {
                  this.$router.replace('/information/reservoir');
                }, 1500);
              }
            })
          }
      ...
      drawEnd(e) {
        ...
        this.form.setFieldsValue({
          centerLongitude: centerPoint[0],
          centerLatitude: centerPoint[1],
