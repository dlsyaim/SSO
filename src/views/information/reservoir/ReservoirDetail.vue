  ...
  <div class="card-container">
    <a-card title="湖泊详情" :bordered="false" style="min-height: 100%">
      ...
      <a-row class="modal-detail-item" style="margin-top: 30px">
        <a-col span="6">
          <span class="detail-item-label">水库名称：</span>
          <span>{{detail.reservoirName}}</span>
        </a-col>
        <a-col span="6">
        ...
        <a-col span="6">
          <span class="detail-item-label">水库类型：</span>
          <span>{{detail.reservoirTypeName}}</span>
        </a-col>
      </a-row>
      <a-row class="modal-detail-item" style="margin-top: 30px">
        <a-col span="6">
          <span class="detail-item-label">水库岸别：</span>
          <span>{{detail.reservoirShoreName}}</span>
        </a-col>
        <a-col span="6">
          <span class="detail-item-label">水库面积：</span>
          <span>{{detail.acreage}}</span>
        </a-col>
        <a-col span="6">
          <span class="detail-item-label">经度：</span>
          <span>{{detail.centerLongitude}}</span>
        </a-col>
        <a-col span="6">
          <span class="detail-item-label">纬度：</span>
          <span>{{detail.centerLatitude}}</span>
        </a-col>
      </a-row>
      <a-row class="modal-detail-item" style="margin-top: 30px">
        <a-col span="6">
          <span class="detail-item-label">水库库容：</span>
...
<script>
  ...
  export default {
    ...
    methods: {
      getDetail(id) {
        get(`${BASE_URL}/watersource/v1/reservoir/detail?id=${id}`).then(res => {
          if (res.resCode === 1) {
