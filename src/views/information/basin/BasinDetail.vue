<template>
  <div class="card-container">
    <a-card title="流域详情" :bordered="false" style="min-height: 100%">
      ...
      <a-row class="modal-detail-item" style="margin-top: 30px">
        <a-col span="6">
          <span class="detail-item-label">流域名称：</span>
          <span>{{detail.basinName}}</span>
        </a-col>
        <a-col span="6">
          <span class="detail-item-label">流域代号：</span>
          <span>{{detail.basinCode}}</span>
        </a-col>
      </a-row>
      <a-row class="modal-detail-item" style="margin-top: 30px">
        <a-col span="6">
          <span class="detail-item-label">流域面积：</span>
...
<script>
  ...
  export default {
    ...
    methods: {
      getDetail(id) {
        get(`${BASE_URL}/watersource/v1/drainageBasin/detail?id=${id}`).then(res => {
          if (res.resCode === 1) {
      ...
      initMap() {
        ...
        if(pointString){
          ...
          this.map.centerAndZoom(new T.LngLat(pointArray[0][0], pointArray[0][1]), 9);
