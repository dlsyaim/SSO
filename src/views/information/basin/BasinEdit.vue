              ...
              <a-col span="8">
                <a-form-item v-bind="formLayout" label="流域名称">
                  <a-input placeholder="请输入流域名称"
                           v-decorator="['basinName',{rules:[{required:true,message:'请输入流域名称'}]}]"></a-input>
                </a-form-item>
              </a-col>
              <a-col span="8">
...
<script>
  ...
  export default {
    ...
    methods: {
      getDetail(){
        get(`${BASE_URL}/watersource/v1/drainageBasin/detail?id=${this.id}`).then(res=>{
          if(res.resCode===1){
            const data=res.data;
      ...
      save() {
        this.form.validateFields((err, value) => {
          if (!err) {
            ...
            post(`${BASE_URL}/watersource/v1/drainageBasin/update`, null, data).then(res => {
