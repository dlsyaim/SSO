<template>
  <div class="card-container">
    <a-card title="湖泊管理" :bordered="false" style="min-height: 100%">
      <div style="display: flex;align-items: center;justify-content: space-between">
        <div style="display: flex;align-items: center">
          <span>湖泊名称：</span>
          <a-input v-model="lakeName" placeholder="请输入湖泊名称" class="default-select-width"
                   style="margin-right: 40px"></a-input>
          <span>湖泊类型：</span>
          <a-select v-model="riverType" placeholder="请选择湖泊类型" class="default-select-width" style="margin-right: 40px">
            <a-select-option v-for="item in lakeTypeList" :key="item.id" :value="item.id">{{item.typeName}}</a-select-option>
          </a-select>
          <span>所属区域：</span>
        ...
        <a-button type="primary" @click="$router.push('/information/lake/add')">
...
<script>
  ...
  const columns = [
    ...
    {title: '湖泊名称', dataIndex: 'lakesName'},
    {title: '湖泊岸别', dataIndex: 'lakeShoreName'},
    {title: '湖泊类型', dataIndex: 'lakesTypeName'},
    {title: '所属区域', dataIndex: 'regionName'},
    {title: '湖泊面积', dataIndex: 'acreage'},
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        get(`${BASE_URL}/watersource/v1/lakes/list`, params).then(res => {
          this.loading = false;
          if (res.resCode === 1) {
      ...
      deleteItem() {
        this.loading=true;
        deleteRequest(`${BASE_URL}/watersource/v1/lakes/delete?id=${this.selected.id}`).then(res=>{
          this.loading=false;
          if(res.resCode===1){
