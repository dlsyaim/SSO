        ...
        <div style="width: 70%;" :style="{display:isMapMax?'none':'block'}">
          <a-form :form="form">
            <a-row>
              <a-col span="8">
                <a-form-item v-bind="formLayout" label="湖泊名称">
                  <a-input placeholder="请输入湖泊名称" readOnly
                           v-decorator="['lakesName',{rules:[{required:true,message:'请输入湖泊名称'}]}]"></a-input>
                </a-form-item>
              </a-col>
              <a-col span="8">
            ...
            <a-form-item v-bind="{labelCol: {span: 2},wrapperCol: {span: 21}}" label="管理人员">
              <ManagerForm v-for="item in formList" :key="item.id" ref="manageForm" :form-id="item.id" @deleteForm="deleteForm" :initial-value="item.initialValue"></ManagerForm>
...
<script>
  ...
  export default {
    ...
    methods: {
      getDetail(){
        get(`${BASE_URL}/watersource/v1/lakes/detail?id=${this.id}`).then(res=>{
          if(res.resCode===1){
            const data=res.data;
      ...
      save() {
        ...
        this.form.validateFields((err, value) => {
          if (!err&&!managerFormHasError) {
            ...
            post(`${BASE_URL}/watersource/v1/lakes/update`, null, data).then(res => {
