<template>
  <div class="card-container">
    <a-card title="新增水系" :bordered="false" style="min-height: 100%">
              ...
              <a-col span="8">
                <a-form-item v-bind="formLayout" label="水系名称">
                  <a-input placeholder="请输入水系名称"
                           v-decorator="['waterName',{rules:[{required:true,message:'请输入水系名称'},waterNameAsyncValidator],validateFirst:true,validateTrigger:'blur'}]"></a-input>
                </a-form-item>
              </a-col>
              <a-col span="8">
...
<script>
  ...
  const waterNameAsyncValidator = {
    validator: (rules, value, callback) => {
      get(`${BASE_URL}/watersource/v1/waterSystem/isRepeat?name=${value}`).then(res => {
        if (res.resCode !== 1) {
          callback(new Error());
        }else {
          callback();
      ...
      save() {
        ...
        this.form.validateFields((err, value) => {
          if (!err) {
            ...
            post(`${BASE_URL}/watersource/v1/waterSystem/add`,null,data).then(res=>{
