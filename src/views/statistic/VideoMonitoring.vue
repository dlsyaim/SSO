<template>
  <div class="card-container">
    <a-card title="视频监控预警分析统计" :bordered="false" style="min-height: 100%">
      <div style="display: flex;align-items: center">
        ...
        <a-select v-model="videoId" class="default-select-width" style="margin-right: 40px" placeholder="请选择预警内容">
          <a-select-option v-for="item in videoList" :value="item.id" :key="item.id">{{item.videoName}}</a-select-option>
        </a-select>
        <span>预警状态：</span>
...
<script>
  ...
  const columns=[
    ...
    {title:'预警摄像头名称',dataIndex:'videoName'},
  ...
  export default {
    ...
    methods:{
      ...
      getList(){
        ...
        axios({
          method: 'post',
          url: 'http://122.224.129.35:9088/v7/warning/getLikeList',
          headers: {
      ...
      getVideoList(){
        axios({
          method: 'post',
          url: 'http://122.224.129.35:9088/v7/video/getVideoList',
          headers: {
            'Access-Token': sessionStorage.getItem('Access-Token')
          }
        }).then(res => {
